    ...
    private void scanTarget(long generation, String target, ScanLaunchSpec spec, int profile, boolean allSni,
        ...
        if (ips.isEmpty()) {
            addResult(generation, MainActivity.Result.down(target, "", 0, "", "dns_failed")
                    .attachTargetPlan(TargetPlanRecord.forIpFirstProbe(target, "", 0, "", spec.sniPairingEnabled, expansion)),
        ...
        for (String ip : ips) {
            ...
            for (int port : spec.ports) {
                if (profile == 0) {
                    ...
                    base.attachTargetPlan(TargetPlanRecord.forIpFirstProbe(target, ip, port, "", spec.sniPairingEnabled, expansion));
                ...
                for (String sni : candidates) {
                    ...
                    result.attachTargetPlan(TargetPlanRecord.forIpFirstProbe(target, ip, port, routeSni, spec.sniPairingEnabled, expansion));
